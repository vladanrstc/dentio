...
@section('content')
    <section class="grid cols-3">
        <article class="panel">
            <h3>Pacijenti</h3>
            <div class="metric">{{ $summary['patients_total'] }}</div>
            <div class="muted">Ukupno unetih pacijenata</div>
        </article>

        <article class="panel">
            <h3>Aktivne stavke</h3>
            <div class="metric">{{ $summary['patients_with_open_tasks'] }}</div>
            <div class="muted">Pacijenti sa otvorenim zadacima</div>
        </article>

        <article class="panel">
            <h3>Danasnji termini</h3>
            <div class="metric">{{ $summary['appointments_today'] }}</div>
            <div class="muted">Zakazano za danas</div>
        </article>

        <article class="panel">
            <h3>Podsetnici za slanje</h3>
            <div class="metric">{{ $summary['reminders_due'] }}</div>
            <div class="muted">Ceka slanje email podsetnika</div>
        </article>

        <article class="panel">
            <h3>Dugovanja</h3>
            <div class="metric">{{ number_format($summary['outstanding_amount'], 2) }} RSD</div>
